...
select
  ...
  case when ds_index.idx_dis_pnt_cnts <> 0 and stat.idx_pat_cnt = ds_index.idx_dis_pnt_cnts then 'N' else 'Y' end as index_dup,
  nvl(ds_Doc.doc_ds_pat_cnt, 0) doc_ds_pat_cnt,
  case when  ds_Doc.doc_ds_pat_cnt <> 0 and stat.doc_pat_cnt = ds_Doc.doc_ds_pat_cnt then 'N' else 'Y' end as Doc_dup,
